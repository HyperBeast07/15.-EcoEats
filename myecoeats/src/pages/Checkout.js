...
import { FaNotEqual, FaRegTrashAlt } from "react-icons/fa";
...
import Billing from "../components/utilities/Billing";
...
const Checkout = () => {
  ...
  const handleRemoveFromCart = (productId) => {
    // removing item from context
    removeFromCart(productId);

    // update the local cartData state (for UI)
        ...
        <div className="w-3/5 bg-slate-100 p-8 m-4 rounded-lg">
          ...
          <div className="flex flex-col justify-start text-2xl gap-4">
            <div className="font-bold">
              Total: RM {calculatorTotalPrice().toFixed(2)}
            </div>
            ...
            >
              {isPurchasing ? "Purchasing now..." : "Purchase Now"}
            </button>
          </div>
        </div>
      </div>
